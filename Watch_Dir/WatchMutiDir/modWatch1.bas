...
Public Sub StartWatch1()
UseACollect = True
Do
   For i = 0 To nBufLen: cBuffer(i) = 0: Next i
   Call ReadDirectoryChangesW(DirHndl(0), cBuffer(0), nBufLen, WSubFolder(0), FILE_NOTIF_GLOB, nReadLen, 0, 0)
   Action = cBuffer(4)
   ...
   Select Case Action
            ...
            Case Else
                sAction = "Unknown"
   End Select
   fName = sAction + "-" + DirPath(0) + FileName
   If sAction <> "Unknown" Then
   
    If UseACollect Then  'I can use CollectA1
        CollectA1.Add fName
        If CB1ReadyToRead = False Then CA1ReadyToRead = True: UseACollect = False
    Else             'I can use CollectB1
        CollectB1.Add fName
        If CA1ReadyToRead = False Then CB1ReadyToRead = True: UseACollect = True
    End If
   End If
  Loop
End Sub
